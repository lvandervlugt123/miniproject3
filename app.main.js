...
function createCard(p){
  ...
  const img = document.createElement('img'); img.alt = p.name; img.src=''; img.style.opacity=.95
  ...
  fetchDetails(p.url).then(d=>{
    ...
    tbox.innerHTML = '';
    d.types.forEach(t=>{
      const span = document.createElement('span'); span.className = 'type ' + t.type.name; span.textContent=t.type.name; span.style.background = typeColors[t.type.name]||'#999'; span.style.textTransform='capitalize';
...
function renderList(list){
  ...
  list.forEach((item, idx)=>{
    ...
    setTimeout(()=> c.classList.add('appear'), idx * 40);
  });
}
...
function addToTeam(name){
  ...
  // animate the newly added slot
  setTimeout(()=>{
    const slots = document.querySelectorAll('#teamSlots .slot');
    for(const s of slots){ if(s.textContent.toLowerCase().includes(name.toLowerCase())){ s.classList.add('pulse'); setTimeout(()=>s.classList.remove('pulse'),800); break; }}
  },120);
}
    ...
    content.innerHTML = `
      <div style="display:flex;gap:18px;align-items:center">
        ...
        <div>
          ...
          <div style="margin-top:8px">${d.types.map(t=>`<span class="type ${t.type.name}" style="background:${typeColors[t.type.name]}">${t.type.name}</span>`).join('')}</div>
...
function setup(){
  ...
  $('saveTeam').addEventListener('click',()=>{ 
    ...
    setTimeout(()=>{ btn.classList.remove('saved'); btn.textContent = prev; },1200);
  });
